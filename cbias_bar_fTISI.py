...
for ISI in ISIvals:
	print(ISI)
	performvals = np.loadtxt("data_processed/Performance_%s_%s_ISI%s_ITI%s.txt"%(whichsubject, whichdistrib, ISI, whichITI))
...
axs.bar(ISIvals,[pos[i]*100 for i in range(len(pos))], color=cp.green)
axs.bar(ISIvals,[neg[i]*100 for i in range(len(neg))], color=cp.orange)
# axs.set_ylim(-15,15)
...
fig.savefig('figs/cbias_fISI_%s_%s.png'%(whichsubject, whichdistrib),bbox_inches='tight')
fig.savefig('figs/cbias_fISI_%s_%s.svg'%(whichsubject, whichdistrib),bbox_inches='tight')
