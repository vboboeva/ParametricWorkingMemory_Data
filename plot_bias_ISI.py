...
def main():
	...
	for i, ISI in enumerate(ISIvals):
		...
		for t in range(1,trialsbacks):
			string = "%s_%s_ISI%s_ITI%s_trialsback%d"%(whichsubject, whichdistrib, ISI, whichITI, t)
	...
	# plt.legend()
	fig.savefig("figs/bias_%s_fISI.png"%(string), bbox_inches='tight')
	fig.savefig("figs/bias_%s_fISI.svg"%(string), bbox_inches='tight')
